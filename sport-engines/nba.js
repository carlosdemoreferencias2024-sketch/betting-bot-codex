(function initNbaEngine(root, factory) {
  const api = factory();
  root.BotSportEngines = root.BotSportEngines || {};
  root.BotSportEngines.nba = api;
  if (typeof module !== "undefined" && module.exports) {
    module.exports = api;
  }
}(typeof globalThis !== "undefined" ? globalThis : window, function createNbaEngine() {
  function buildTips({ favorite, totalConfidence, homeEdge, awayEdge, profile }) {
    const teamTotalSide = favorite;
    return [
      {
        type: "Team total",
        pick: `${teamTotalSide} over 112.5 ${profile.totalLabel}`,
